                ...
                <div class="card border-0 rounded-0">
                    ...
                    <?php if (empty($posts)): ?>
                        <div class="col-md-8 mx-auto py-5">
                            <h1 class="fs-2 mb-2 text-center">Aucun article pour le moment</h1>
                        </div>
                    <?php else: ?>
                        <div class="card-body border-top border-orange">
                            <table class="table table-striped table-hover">
                                ...
                                <?php foreach ($posts as $post): ?>
                                    ...
                                    ?>
                                    <tr>
                                        <td ><?= $post->id ?></td scope="row">
                                        <td class="fw-bold">
                                            <a href="single.php?id=<?= $post->id ?>" style="height: 80px; overflow: hidden; display:block;">
                                                <h6><?= $post->title ?></h6>
                                                <img src="<?= $post->image ?>" alt="" width="80">
                                            </a>

                                        </td>
                                        <td style="width: 360px;"><?= excerpt($post->content, 0, 100) ?></td>
                                        <td>
                                            <?php foreach ($categories as $category): ?>
                                                <span class="badge rounded-pill bg-dark"><?= $category->title ?></span>
                                             <?php endforeach;?>
                                        </td>
